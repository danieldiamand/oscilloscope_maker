...
from rotateVertex import rotateVertex

objFileName = "cube"
saveFileName = "cube"
frameRate = 44100
initRotation = [0,0,-1.5708]
rotationArray = [0,0,0] #in radians
...
class Line:
    def __init__(self, x1,y1,x2,y2, speedLimit=0.025):
        self.x1 = x1*scale
        self.y1 = y1*scale
        self.x2 = x2*scale
        self.y2 = y2*scale
        self.drawFrames = int(math.hypot(self.x2-self.x1, self.y2-self.y1)/speedLimit) #how many frames it takes to draw line, ie how long it takes to move between point 1 and 2 
        self.xStep = (self.x2-self.x1)/self.drawFrames
        self.yStep = (self.y2-self.y1)/self.drawFrames


vertices, edges = readObj(objFileName)
vertices = [rotateVertex(vertex, initRotation) for vertex in vertices]
lineArray = []
    ...
    for _ in range(pausePerRotate):
        for line in (lineArray):
            for i in range(line.drawFrames):
                rightChannel.append(line.x1+(line.xStep*i))
                leftChannel.append(line.y1+(line.yStep*i))
    
    vertices = [rotateVertex(vertex, rotationArray) for vertex in vertices]
